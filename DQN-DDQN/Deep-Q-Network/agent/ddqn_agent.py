...
class DDQNAgent:
    ...
    def learn(self, experiences, gamma):
        ...
        v_st1 = self.target_network.model.predict(next_states)
        self.Q_targets = self.local_network.model.predict(states)
        for index, done in enumerate(dones):
            if done:
                self.Q_targets[index][actions[index]] = rewards[index]
            else:
                self.Q_targets[index][actions[index]] = (rewards + gamma * np.amax(v_st1, axis=1))[index]

        history = self.local_network.model.fit(states, self.Q_targets, epochs=1, verbose=0, callbacks=self.callbacks)
