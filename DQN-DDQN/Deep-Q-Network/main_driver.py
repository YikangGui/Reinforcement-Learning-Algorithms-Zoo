...
def train_model(n_episodes=2000, eps_start=1.0, eps_end=0.001, eps_decay=0.9, target_reward=1000):
    ...
    for i_episode in range(1, n_episodes + 1):
        ...
        logger.debug('Episode {}\tAverage Score: {:.2f}\tScore: {:.2f}\tLoss: {:.2f}\tState: {}\tMean Q-Target: {:.4f}'
                     '\tEffective Epsilon: {:.3f}\tTime Taken: {:.2f} sec'.format(
            i_episode, np.mean(scores_window), score, np.mean(losses), state[0], np.mean(agent.Q_targets), eps, time_taken))
        if i_episode % 100 == 0:
            result_logger.info(
