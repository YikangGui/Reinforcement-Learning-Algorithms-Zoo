...
Original file is located at
    https://colab.research.google.com/drive/1rgAoHVaX-P2OfjBr4z8e_H22UDow-42D
...
def generate_episode(env, policy):
  ...
  while not done:
      # WRITE CODE HERE
      states.append(state)
      action = np.argmax(policy.predict(np.array(state).reshape(-1,4)))
...
ok = (subprocess.check_output(['md5sum', 'expert.h5']).split()[0] == b'059f8e9c77de294e1a9d5e6af8825c39')
...
class Imitation():
    ...
    def generate_dagger_data(self):
        ...
        self._train_actions = []
        for _ in range(self.num_episodes):
          student_states, _, _ = generate_episode(self.env, self.model)
          expert_actions = np.argmax(self.expert.predict(np.array(student_states)), axis=1)
          zeros = np.zeros([len(expert_actions), 2])
          for i in range(len(expert_actions)):
            zeros[i][expert_actions[i]] = 1
          self._train_states.extend(np.array(student_states))
          self._train_actions.extend(zeros)
        self._train_states = np.array(self._train_states)
        self._train_actions = np.array(self._train_actions)
...
for num_episodes in [1, 10, 50, 100]:
  for t in range(random_seeds):
    ...
    for t in range(num_iterations):
      ...
      losses.append(loss)
      accs.append(acc)
      print('(%d) loss = %.3f; accuracy = %.2f; reward = %.1f' % (t, loss, acc, reward))
      
    accuracy_data[num_episodes].append(accs)
    loss_data[num_episodes].append(losses)
    reward_data[num_episodes].append(rewards)

"""Plot the reward, loss, and accuracy for each, remembering to label each line."""

keys = [1, 10, 50, 100]

for i, index in zip(list(map(lambda x: np.mean(reward_data[x]), [1, 10, 50, 100])), [1, 10, 50, 100]):
    reward_data[index] = i
    
for i, index in zip(list(map(lambda x: np.mean(accuracy_data[x]), [1, 10, 50, 100])), [1, 10, 50, 100]):
    accuracy_data[index] = i

for i, index in zip(list(map(lambda x: np.mean(loss_data[x]), [1, 10, 50, 100])), [1, 10, 50, 100]):
    loss_data[index] = i
...
class CMAES:
    ...
    def populate(self):
        ...
        for _ in range(self.n):
          self.population.append(np.random.multivariate_normal(mean=self.mu, cov=self.S + np.eye(self.d) * self.noise))
#         print(np.random.multivariate_normal(mean=self.mu, cov=self.S + np.eye(self.d) * self.noise).shape)
    ...
    def evaluate_policy(self, policy, num_episodes):
        ...
        for episode in range(num_episodes):
          ...
          if self.reward_fn == None:
            ...
            states, _, _ = generate_episode(self.env, policy)
            for state in states:
              reward = self.reward_fn(state, None)
    ...
    def train(self):
        ...
        # WRITE CODE HERE
        population_rewards = []
        best_member_num = int(self.p*self.n)
        for i in range(len(self.population)):
          reward = self.evaluate(self.population[i], self.L)
          population_rewards.append(reward)
        indices = np.argsort(-1*np.array(population_rewards))[:best_member_num]
        
        sums = 0.0
        best_member = self.population[indices[0]]
        best_members = []
        for index in indices:
          sums += self.population[index]
          best_members.append(self.population[index])
        mean = sums/best_member_num
        
        self.mu = mean
        self.S = np.cov(np.array(best_members).T)
...
mu_r = np.array(data[50])[:, 0]  # Use the performance of the best point
best_r = np.array(data[50])[:, 1]  # Use the performance of the best point

x = np.arange(len(mu_r)) + 1
plt.plot(x, best_r, label="best reward")
plt.plot(x, mu_r, label="average reward")
...
#       if collect_expert:
      expert_state, _, _ = generate_episode(self.env, self.expert)
#         self.expert_states.extend(list(map(lambda x:x[None], expert_state)))
#         self.expert_y.extend(list(map(lambda x:np.array([[1., 0]]), expert_state)))
      self.expert_states.extend(expert_state)
      self.expert_y.extend(list(map(lambda x:np.array([1., 0]), expert_state)))
        
      student_state, _, _ = generate_episode(self.env, self.cmaes.model)
#       self.student_states.extend(list(map(lambda x:x[None], student_state)))
#       self.student_y.extend(list(map(lambda x:np.array([[0, 1.]]), student_state)))
      self.student_states.extend(student_state)
      self.student_y.extend(list(map(lambda x:np.array([0., 1]), student_state)))
      
    assert len(self.expert_states) == len(self.expert_y)
    assert len(self.student_states) == len(self.student_y)

  def train_discriminator(self):
    # WRITE CODE HERE
    self.collect_data(self.num_episodes) 
    X = self.expert_states + self.student_states
    Y = self.expert_y + self.student_y
...
accs = []
for _ in range(10):
  acc = gail.train_discriminator()[1]
  accs.append(acc)
  print(acc)

plt.figure(figsize=(4, 4))
plt.plot(range(1,len(accs)+1), accs, label="acc")
...
plt.xlabel('number of iterations', fontsize=16)
plt.ylabel("expert probability", fontsize=16)

plt.subplot(133)
plt.plot(range(1,len(tv)+1), tv)
plt.xlabel('number of iterations', fontsize=16)
plt.ylabel("TV-distance", fontsize=16)   

plt.savefig('GAIL_cmaes.png', dpi=300)
plt.show()
...
for t in range(100):
  ...
  if t % discriminator_update_period == 0:
    gail.train_discriminator()
  gail.collect_data(gail.num_episodes)
  gail.train_policy()
